...
SSDDataTransformer<Dtype>::SSDDataTransformer(const TransformationParameter& param, Phase phase) : DataTransformer<Dtype>(param, phase) 
    {
    }

// Overide this function from original 
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
template<typename Dtype>
void SSDDataTransformer<Dtype>::Transform(const Datum& datum,
...
template<typename Dtype>
vector<int> SSDDataTransformer<Dtype>::InferBlobShape(const Datum& datum) {
  if (datum.encoded()) {
#ifdef USE_OPENCV
    CHECK(!(this->param_.force_color() && this->param_.force_gray()))
        << "cannot set both force_color and force_gray";
    cv::Mat cv_img;
    if (this->param_.force_color() || this->param_.force_gray()) {
    // If force_color then decode in color otherwise decode in gray.
      cv_img = DecodeDatumToCVMat(datum, this->param_.force_color());
    } else {
  ...
  int crop_h = this->param_.crop_h();
  int crop_w = this->param_.crop_w();
  if (crop_size) {
    crop_h = crop_size;
    crop_w = crop_size;
  }
  ...
  if (this->param_.has_resize_param()) {
    InferNewSize(this->param_.resize_param(), datum_width, datum_height,
...
#ifdef USE_OPENCV
template<typename Dtype>
void SSDDataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob) {
  NormalizedBBox crop_bbox;
  bool do_mirror;
...
template<typename Dtype>
vector<int> SSDDataTransformer<Dtype>::InferBlobShape(const cv::Mat& cv_img) {
  const int crop_size = this->param_.crop_size();
  int crop_h = this->param_.crop_h();
  int crop_w = this->param_.crop_w();
  if (crop_size) {
  ...
  if (this->param_.has_resize_param()) {
    InferNewSize(this->param_.resize_param(), img_width, img_height,
...
// end overided functions
//----------------------------------------------------------------------------------------------------------------
template<typename Dtype>
void SSDDataTransformer<Dtype>::Transform(
...
//----------------------------------------------------------------------------------
template<typename Dtype>
void SSDDataTransformer<Dtype>::TransformAnnotation(
  ...
  if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX) {
    ...
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
        ...
        if (do_resize && this->param_.has_resize_param()) {
          CHECK_GT(img_height, 0);
          CHECK_GT(img_width, 0);
          UpdateBBoxByResizePolicy(this->param_.resize_param(), img_width, img_height,
                                   &resize_bbox);
        }
        if (this->param_.has_emit_constraint() &&
            !MeetEmitConstraint(crop_bbox, resize_bbox,
                                this->param_.emit_constraint())) {
          continue;
        }
        ...
        if (ProjectBBox(crop_bbox, resize_bbox, &proj_bbox)) {
          ...
          if (do_resize && this->param_.has_resize_param()) {
            ExtrapolateBBox(this->param_.resize_param(), img_height, img_width,
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::CropImage(const Datum& datum,
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::CropImage(const AnnotatedDatum& anno_datum,
                                       const NormalizedBBox& bbox,
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::ExpandImage(const Datum& datum,
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
                                         AnnotatedDatum* expanded_anno_datum) {
  if (!this->param_.has_expand_param()) {
    expanded_anno_datum->CopyFrom(anno_datum);
    return;
  }
  const ExpansionParameter& expand_param = this->param_.expand_param();
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::DistortImage(const Datum& datum,
                                          Datum* distort_datum) {
  if (!this->param_.has_distort_param()) {
  ...
  if (datum.encoded()) {
    ...
    // Distort the image.
    cv::Mat distort_img = ApplyDistort(cv_img, this->param_.distort_param());
    // Save the image into datum.
...
void SSDDataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
  ...
  *do_mirror = this->param_.mirror() && this->Rand(2);
  ...
  if (this->param_.has_resize_param()) {
    cv_resized_image = ApplyResize(cv_img, this->param_.resize_param());
  } else {
    cv_resized_image = cv_img;
  }
  if (this->param_.has_noise_param()) {
    cv_noised_image = ApplyNoise(cv_resized_image, this->param_.noise_param());
  } else {
  ...
  if (has_mean_file) {
    CHECK_EQ(cv_cropped_image.rows, this->data_mean_.height());
    CHECK_EQ(cv_cropped_image.cols, this->data_mean_.width());
  }
...
template <typename Dtype>
void SSDDataTransformer<Dtype>::CropImage(const cv::Mat& img,
                                       const NormalizedBBox& bbox,
                                       cv::Mat* crop_img) {
...
template <typename Dtype>
void SSDDataTransformer<Dtype>::ExpandImage(const cv::Mat& img,
                                         const float expand_ratio,
                                         NormalizedBBox* expand_bbox,
                                         cv::Mat* expand_img) {
  const int img_height = img.rows;
  ...
  if (has_mean_file) {
    CHECK_EQ(img_channels, this->data_mean_.channels());
    CHECK_EQ(height, this->data_mean_.height());
    CHECK_EQ(width, this->data_mean_.width());
    Dtype* mean = this->data_mean_.mutable_cpu_data();
    for (int h = 0; h < height; ++h) {
  ...
  if (has_mean_values) {
    ...
    CHECK_EQ(channels.size(), this->mean_values_.size());
    for (int c = 0; c < img_channels; ++c) {
      channels[c] = this->mean_values_[c];
    }
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::TransformInv(const Blob<Dtype>* blob,
...
template<typename Dtype>
void SSDDataTransformer<Dtype>::TransformInv(const Dtype* data, cv::Mat* cv_img,
  ...
  Dtype* mean = NULL;
  if (has_mean_file) {
    CHECK_EQ(channels, this->data_mean_.channels());
    CHECK_EQ(height, this->data_mean_.height());
    CHECK_EQ(width, this->data_mean_.width());
    mean = this->data_mean_.mutable_cpu_data();
  }
  if (has_mean_values) {
    CHECK(this->mean_values_.size() == 1 || this->mean_values_.size() == channels) <<
        "Specify either 1 mean_value or as many as channels: " << channels;
    if (channels > 1 && this->mean_values_.size() == 1) {
      // Replicate the mean_value for simplicity
    ...
    for (int w = 0; w < width; ++w) {
      for (int c = 0; c < channels; ++c) {
        ...
        if (has_mean_file) {
          ...
          if (has_mean_values) {
            ptr[img_idx++] =
                static_cast<uchar>(data[idx] / scale + this->mean_values_[c]);
          } else {
  ...
  if (this->param_.has_resize_param()) {
    *cv_img = ApplyResize(orig_img, this->param_.resize_param());
  } else {
    *cv_img = orig_img;
  }
}
...
void SSDDataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  const int crop_size = this->param_.crop_size();
  const Dtype scale = this->param_.scale();
  *do_mirror = this->param_.mirror() && Rand(2);
  const bool has_mean_file = this->param_.has_mean_file();
  ...
  Dtype* mean = NULL;
  if (has_mean_file) {
    CHECK_EQ(datum_channels, this->data_mean_.channels());
    CHECK_EQ(datum_height, this->data_mean_.height());
    CHECK_EQ(datum_width, this->data_mean_.width());
    mean = this->data_mean_.mutable_cpu_data();
  }
  if (has_mean_values) {
    CHECK(this->mean_values_.size() == 1 || this->mean_values_.size() == datum_channels) <<
     "Specify either 1 mean_value or as many as channels: " << datum_channels;
    if (datum_channels > 1 && this->mean_values_.size() == 1) {
      // Replicate the mean_value for simplicity
    ...
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        if (has_mean_file) {
          ...
          if (has_mean_values) {
            transformed_data[top_index] =
              (datum_element - this->mean_values_[c]) * scale;
          } else {
...
template <typename Dtype>
int SSDDataTransformer<Dtype>::Rand(int n) {
  CHECK(this->rng_);
  CHECK_GT(n, 0);
  caffe::rng_t* rng =
...
INSTANTIATE_CLASS(SSDDataTransformer);
