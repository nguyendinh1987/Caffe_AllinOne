...
#include "caffe/SSD/annotated_data_layer.hpp"
#include "caffe/util/benchmark.hpp"
#include "caffe/SSD/sampler.hpp"

namespace caffe {

template <typename Dtype>
AnnotatedDataLayer<Dtype>::AnnotatedDataLayer(const LayerParameter& param)
  : BaseDataLayer<Dtype>(param),prefetch_free_(), prefetch_full_(), reader_(param){
...
template <typename Dtype>
void AnnotatedDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  ssd_data_transformer_.reset(
      new SSDDataTransformer<Dtype>(this->transform_param_, this->phase_));
  ssd_data_transformer_->InitRand();
  ...
  // this->ssd_data_transformer_->InitRand();
  StartInternalThread();
...
template <typename Dtype>
void AnnotatedDataLayer<Dtype>::InternalThreadEntry() {
#ifndef CPU_ONLY
  cudaStream_t stream;
  if (Caffe::mode() == Caffe::GPU) {
...
void AnnotatedDataLayer<Dtype>::DataLayerSetUp(
  ...
  vector<int> top_shape =
      this->ssd_data_transformer_->InferBlobShape(anno_datum.datum());
...
void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (transform_param.has_distort_param()) {
      ...
      this->ssd_data_transformer_->DistortImage(anno_datum.datum(),
                                            distort_datum.mutable_datum());
      if (transform_param.has_expand_param()) {
        expand_datum = new AnnotatedDatum();
        this->ssd_data_transformer_->ExpandImage(distort_datum, expand_datum);
      } else {
      ...
      if (transform_param.has_expand_param()) {
        expand_datum = new AnnotatedDatum();
        this->ssd_data_transformer_->ExpandImage(anno_datum, expand_datum);
      } else {
    ...
    if (batch_samplers_.size() > 0) {
      ...
      if (sampled_bboxes.size() > 0) {
        ...
        this->ssd_data_transformer_->CropImage(*expand_datum,
    ...
    timer.Start();
    vector<int> shape =
        this->ssd_data_transformer_->InferBlobShape(sampled_datum->datum());
    if (transform_param.has_resize_param()) {
      if (transform_param.resize_param().resize_mode() ==
          ResizeParameter_Resize_mode_FIT_SMALL_SIZE) {
    ...
    if (this->output_labels_) {
      if (has_anno_type_) {
        ...
        this->ssd_data_transformer_->Transform(*sampled_datum,
        ...
        this->ssd_data_transformer_->Transform(sampled_datum->datum(),
                                           &(this->transformed_data_));
...
// Get from base_data_layer.cpp in SSD implementation
template <typename Dtype>
void AnnotatedDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(AnnotatedDataLayer, Forward);
#endif
