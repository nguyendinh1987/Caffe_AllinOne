...
void Deconvolution3DLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  col_shape[0] = 1;
  col_shape[1] = num_output_ * kernel_depth_ * kernel_size_ * kernel_size_;
  col_shape[2] = length_;
  col_shape[3] = height_;
  col_shape[4] = width_;
...
void Deconvolution3DLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < num_; ++n) {
    offset_indices[0] = n;
	  vol2col_cpu(top_diff + top[0]->offset(offset_indices), num_output_, length_out_, height_out_, width_out_,
